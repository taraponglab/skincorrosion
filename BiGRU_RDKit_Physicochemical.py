...
def train_bigru_model(x_train, x_test, y_train, y_test, model_filename='bigru.keras', epochs=50, batch_size=2):
    ...
    # Reshape data for BiGRU: (samples, timesteps, features)
...
def main():
    # Load data from CSV
    x_train = pd.read_csv("corrosion_x_train_rdkit_descriptors.csv",index_col=0)
    x_test = pd.read_csv("corrosion_x_test_rdkit_descriptors.csv",index_col=0)
    y_train = pd.read_csv("corrosion_y_train_numeric.csv",index_col=0)
    y_test = pd.read_csv("corrosion_y_test_numeric.csv",index_col=0)
    accuracy_train, accuracy_test,mcc_train,mcc_test,precision_train,precision_test,recall_train,recall_test, specificity_train,specificity_test, roc_auc_test,roc_auc_train, model_file, plot_file = train_bigru_model(x_train, x_test, y_train, y_test)

...
def train_lstm_model(x_train, x_test, y_train, y_test, model_filename='lstm_maccs_descriptors.keras', plot_filename='lstm_maccs_descriptors.png', epochs=50, batch_size=2):
...
def main():
    # Load data from CSV
    x_train = pd.read_csv("corrosion_x_train_maccs_descriptors_shuffled.csv",index_col=0)
    x_test = pd.read_csv("corrosion_x_test_maccs_descriptors_shuffled.csv",index_col=0)
